...
### Оценка по бенчмарку RUSSE-2022 / ruDetox
Дополнительно модель проверена на внешнем публичном бенчмарке для русской детоксификации RUSSE-2022 / ruDetox — с теми же официальными классификаторами, что используются в оригинальном соревновании (`s-nlp/russian_toxicity_classifier` для STA, `s-nlp/rubert-base-corruption-detector` для FL, LaBSE для SIM).

| Метрика | До обучения | После SFT+DPO | Прирост |
|---|---|---|---|
| **STA** (Style Transfer Accuracy) | 0.2935 | 0.5716 | +95% |
| **SIM** (Meaning Preservation) | 0.5981 | 0.7414 | +24% |
| **FL** (Fluency) | 0.4478 | 0.5042 | +13% |
| **J** (Joint score, STA×SIM×FL) | 0.0528 | **0.2181** | **×4.1 (+313%)** |

`J` — основная сводная метрика бенчмарка (среднее по примерам произведение STA, SIM и FL); используется как основной индикатор итогового качества модели для сравнения с другими решениями по детоксификации русского текста. Полный скрипт оценки — `banchmark/evaluate_russe_detox.py` .
